...
def shopify_builder(sticky_note):
    ...
    -------
    dict
        A dictionary containing necessary CSV formats as values (ex: Master, Sku Key, Defaults, etc.)
    """
    ...
    name = df['Title']
    desc = df['Body (HTML)']
    imgs = df['Image Src']                                          # TODO: Format images to only contain one link, not a comma seperated list
    opt1Name, opt1Val = df['Option1 Name'], df['Option1 Value']     # TODO: Handle this steaming pile of garbage...
    ...
    sku = frm.format_skus( df['Handle'], name )             # TODO: Fix SKU func
